...
export async function searchKnowledgeBase(newsTitle, newsContent, maxResults = 5) {
  try {
    // Use FULL news content for semantic search - find teachings that relate to the actual story
    const newsContext = newsContent
      ? `${newsTitle}\n\n${newsContent}`.substring(0, 2000) // Include full content, limit to 2000 chars
      : newsTitle;

    const question = `Find teachings from Maulana Wahiduddin Khan's books that relate to this news story:\n\n${newsContext}\n\nReturn passages about: contemplation (tafakkur), gratitude (shukr), patience (sabr), wisdom, faith, or spiritual lessons that connect to the themes in this news.`;
