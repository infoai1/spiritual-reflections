...
export async function fetchCategorizedNews(options = {}) {
  ...
  if (!apiKey) {
    ...
    return {
      ...
      nature: getSampleNewsByCategory('nature'),
      health: getSampleNewsByCategory('health'),
      all: getSampleNews().slice(0, forAllSection)
    };
  }

  try {
    // Parallel fetch for each category + general news
    const [inspirationNews, scienceNews, natureNews, healthNews, generalNews] = await Promise.all([
      fetchNewsByCategory('inspiration', perCategory * 2),
      fetchNewsByCategory('science', perCategory * 2),
      fetchNewsByCategory('nature', perCategory * 2),
      fetchNewsByCategory('health', perCategory * 2),
    ...
    return {
      ...
      nature: natureNews.slice(0, perCategory),
      health: healthNews.slice(0, perCategory),
...
function getSampleNewsByCategory(categoryId) {
  ...
  if (categoryId === 'nature') {
    // Filter for nature stories (wildlife, ocean, forest, conservation)
    return samples.filter(n => {
      const text = `${n.title} ${n.description}`.toLowerCase();
      return text.includes('nature') ||
  ...
  if (categoryId === 'health') {
    // Filter for health stories (medical, wellness, healing)
    return samples.filter(n => {
      const text = `${n.title} ${n.description}`.toLowerCase();
